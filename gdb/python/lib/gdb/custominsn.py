...
class Insn:
    ...
    def expand_format_string(self, fields, info):
        ...
        if 'imm' in fields and 'dest' not in fields:
            ...
            fields['dest'] \
                 = gdb.format_address(addr, info.progspace, info.architecture)
...
class RISCVDisassembler(SyntaxHighlightingDisassembler):
    ...
    def __call__(self, info):
        ...
        if str is not None:
            ...
            return gdb.disassembler.DisassemblerResult(len(insn_bytes), str)
        else:
            builtin_result = gdb.disassembler.builtin_disassemble(info)
            length = builtin_result.length
            str = builtin_result.string
            return gdb.disassembler.DisassemblerResult(length, str)
